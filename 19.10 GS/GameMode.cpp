...
void GameMode::ReadyToStartMatch(UObject* Context, FFrame& Stack, bool* Ret) {
	...
	if (!GameMode->bWorldIsReady) {
		...
		if (!bDev)
		{
			API::GameServerAsync("http://185.206.149.110:45011/solstice/api/v1/matchmaking/start/by-address", IP, 7777);	
		}
	}
...
void GameMode::HandleStartingNewPlayer(UObject* Context, FFrame& Stack) {
	...
	if (bCreative && !PlayerState->bIsSpectator)
	{
		...
		if (Portal)
		{
			...
			Portal->IslandInfo.SupportCode = L"BenziGS";
			...
			Creative::SpawnSettingsMachine(Portal->GetLinkedVolume());
...
void GameMode::OnAircraftEnteredDropZone(UObject* Context, FFrame& Stack) {
	...
	if (!bDev)
	{
		API::GameServerAsync("http://185.206.149.110:45011/solstice/api/v1/matchmaking/stop/by-address", IP, 7777);	
	}
...
void GameMode::Hook()
{
	...
	Utils::ExecHook(L"/Script/FortniteGame.FortGameModeAthena.HandleMatchHasEnded", HandleMatchHasEnded, HandleMatchHasEndedOG);
	if (bGameSessions)
	{
