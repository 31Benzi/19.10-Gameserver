...
void Building::ServerCreateBuildingActor(UObject* Context, FFrame& Stack)
{
    ...
    // Creative Building Permissions
    if (bCreative)
    {
        auto Controller = (AFortPlayerControllerAthena*)PlayerController;
        auto PlayerState = (AFortPlayerStateAthena*)Controller->PlayerState;

        if (!Controller->CreativePlotLinkedVolume)
        {
            PlayerController->ClientSendMessage(UKismetTextLibrary::Conv_StringToText(L"Building is disabled in the Creative Hub!"), nullptr);
            return;
        }

        auto Volume = Controller->CreativePlotLinkedVolume;
        auto LevelSaveComponent = (UFortLevelSaveComponent*)Volume->GetComponentByClass(UFortLevelSaveComponent::StaticClass());
        if (LevelSaveComponent && LevelSaveComponent->PlotPermissions.Permission == EFortCreativePlotPermission::Private)
        {
            if (LevelSaveComponent->AccountIdOfOwner != PlayerState->UniqueId)
            {
                PlayerController->ClientSendMessage(UKismetTextLibrary::Conv_StringToText(L"You don't have permission to build on this island!"), nullptr);
                return;
            }
        }
    }
	...
	if (!PlayerController->bBuildFree && !bCreative)
	{
...
void Building::OnDamageServer(ABuildingSMActor* Actor, float Damage, FGameplayTagContainer DamageTags, FVector Momentum, FHitResult HitInfo, AFortPlayerControllerAthena* InstigatedBy, AActor* DamageCauser, FGameplayEffectContextHandle EffectContext) {
	if (bCreative && InstigatedBy && DamageCauser && DamageCauser->IsA<AFortWeapon>())
	{
		auto Weapon = (AFortWeapon*)DamageCauser;
		if (Weapon->WeaponData && Weapon->WeaponData->IsA<UFortWeaponMeleeItemDefinition>())
		{
...
void Building::ServerSpawnDeco(UObject* Context, FFrame& Stack) {
	...
	if (bCreative)
	{
		...
		if (!PlayerController->CreativePlotLinkedVolume)
		{
			PlayerController->ClientSendMessage(UKismetTextLibrary::Conv_StringToText(L"Building is disabled in the Creative Hub!"), nullptr);
			return;
		}

		auto Volume = PlayerController->CreativePlotLinkedVolume;
		auto LevelSaveComponent = (UFortLevelSaveComponent*)Volume->GetComponentByClass(UFortLevelSaveComponent::StaticClass());
		if (LevelSaveComponent && LevelSaveComponent->PlotPermissions.Permission == EFortCreativePlotPermission::Private)
		{
			if (LevelSaveComponent->AccountIdOfOwner != PlayerState->UniqueId)
